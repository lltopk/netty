...
//因为register方法是被NioEventLoop调用的，而调用的时候又要把调用该方法的NioEventLoop传递进来
//因为一个channel要绑定一个唯一的NioEventLoop，而SingleThreadEventLoop是NioEventLoop的父类
//所以上面的方法其实可以被重构为下面这样
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/b60f40fe370241a89245dfd145855c3a~tplv-k3u1fbpfcp-zoom-1.image)
