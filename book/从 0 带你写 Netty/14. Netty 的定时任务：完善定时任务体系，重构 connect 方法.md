...
public class Test {
    ...
    public static void main(String[] args) {
        //创建一个普通的任务
        NormalRunnable normalRunnable = new NormalRunnable();

        //创建一个线程,启动线程
        Thread thread = new Thread(normalRunnable);
        thread.setName("普通任务线程");
        thread.start();
    }
...
public class NormalRunnable implements Runnable{
    ...
    public NormalRunnable(long time, TimeUnit timeUnit){
    ...
    public void run() {
        try {
            //线程一启动就会执行到这里，然后睡一会，等睡过了用户设定的时间，再去执行任务
            //在这里就是睡了5秒
            Thread.sleep(time);
            System.out.println(Thread.currentThread().getName()+":我现在要去看电影！");
...
public class Test {
    ...
    public static void main(String[] args) {
        //创建一个普通的任务
        NormalRunnable normalRunnable = new NormalRunnable(5000,MILLISECONDS);
...
```Java
//比较器的名字就是SCHEDULED_FUTURE_TASK_COMPARATOR
...
```Java
public final class DefaultPriorityQueue<T> {
    ...
    public DefaultPriorityQueue(Comparator<T> comparator, int initialSize) {
        ...
        //如果用户传进来的初始化长度为0，就把该类的EMPTY_ARRAY成员变量赋值给queue
        //如果不为空，就创建长度为initialSize的定时任务数组，这里为什么要要用数组这个容器呢？
        //后面就会讲解到
...
```Java
public class ScheduledFutureTask implements Runnable，Comparable{
    ...
    public int compareTo(ScheduledFutureTask task) {
        ...
        //获取执行时间的差值
        long d = deadlineNanos() - task.deadlineNanos();
        if (d < 0) {
            return -1;
        } else if (d > 0) {
            return 1;
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/5b11e9edc71d4e6d950a4b2451cb507f~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/a10f4ffe214645b5ab01948352ffbede~tplv-k3u1fbpfcp-zoom-1.image)
...
private void bubbleUp(int k, T node) {
    while (k > 0) {
        ...
        queue[k] = parent;
        //然后用iParent给k赋值，以便寻找父节点的父节点，然后和定时任务的执行时间做对比
        k = iParent;
    }
    //这里把定时任务放在数组对应的索引位置
    queue[k] = node;
...
public class Test {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        //创建一个优先级队列,这里用的还是上面给出的SCHEDULED_FUTURE_TASK_COMPARATOR比较器
...
public  class AbstractScheduledEventExecutor {
    ...
    //向优先级任务队列中添加任务
    ScheduledFutureTask schedule(final ScheduledFutureTask task) {
...
public class Test {
    ...
    public static void main(String[] args) throws InterruptedException {
        //创建一个定时任务，任务逻辑就用我们内部写死的那个，就不再重新定义了
        ScheduledFutureTask task = new ScheduledFutureTask(5000, TimeUnit.NANOSECONDS);
        //创建一个万能调度器
...
private void select() throws IOException {
    ...
    for (;;){
        ...
        //如果有事件或者单线程执行器中有任务待执行，或者有定时任务等待执行，就退出循环
